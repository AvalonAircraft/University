# Module: Lambda3 (S3 Storage Logic)
...
variable "function_name" { 
  type        = string 
  description = "Name der Funktion (z.B. Lambda3)"
}
...
variable "source_file" { 
  ...
  validation {
    ...
    error_message = "Bei 'use_archive=true' muss 'source_file' auf die .py Datei zeigen."
  }
}
...
variable "description"            { type = string, default = "Lambda3 function for S3 Storage" }
...
variable "existing_role_name" { 
  type    = string 
  default = "service-role/Lambda3-role-7t5id6pm" 
}
...
locals {
  ...
  # Logik für konsistente ENV-Variablen
  env_final = merge(
    {
  ...
  # Vorbereitung für Lambda Permissions (API Gateway)
  apigw_perms = flatten([
...
resource "aws_lambda_permission" "apigw_invoke" {
  ...
  # Präzise Source ARN Einschränkung
