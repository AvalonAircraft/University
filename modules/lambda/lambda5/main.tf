...
variable "source_file" { 
  ...
  validation {
    ...
    error_message = "Bei 'use_archive=true' muss 'source_file' auf die Quell-Datei zeigen."
  }
}
...
variable "env" { 
  type    = map(string) 
  default = { DEFAULT_CLIENTS = "", STRICT_MODE = "0" } 
}
...
variable "existing_role_name" { 
  type    = string 
  default = "service-role/Lambda5-role-gmx3qmuy" 
}
...
# Referenz auf bestehende IAM Rolle
...
resource "aws_lambda_function" "fn" {
  ...
  # Abhängigkeit zur Log-Gruppe für sauberes Provisioning
