...
variable "vpc_security_group_ids" { type = list(string) }              # z.B. [sg-...]
...
# KMS (Cluster + Performance Insights)
variable "cluster_kms_key_id" {
  description = "KMS Key ID/Alias for storage encryption (default AWS-managed RDS key)"
  type        = string
  default     = "alias/aws/rds"
}

variable "pi_kms_key_id" {
  description = "KMS Key ID/Alias for Performance Insights encryption"
  type        = string
  default     = "alias/aws/rds"
}
...
locals {
  # Enhanced Monitoring: wenn keine Rolle angegeben ist => Monitoring aus (0)
  monitoring_interval = var.monitoring_role_arn == null ? 0 : 60
}
...
resource "aws_rds_cluster" "this" {
  ...
  # Serverless v2 läuft unter engine_mode = provisioned
  ...
  # Data API (nur falls du es brauchst)
...
resource "aws_rds_cluster_instance" "writer" {
  ...
  # Enhanced Monitoring (nur wenn role gesetzt)
...
output "endpoint_writer" { value = aws_rds_cluster.this.endpoint }
